...
const limitedMinutesPenalty = r.minutes > 0 && r.minutes < 24 ? (24 - r.minutes) * 1.5 : 0;
const benchRunPenalty = r.minutes > 0 && r.minutes < 18 ? 10 : 0;
const lowProjectionFloorPenalty = r.projection > 0 && r.projection < 12 ? (12 - r.projection) * 1.5 : 0;
const adjustedFloorRiskScore = clamp(bustRiskScore + limitedMinutesPenalty + benchRunPenalty + lowProjectionFloorPenalty);
const floorSafetyScore = round(100 - adjustedFloorRiskScore);
...
return {...r, slateFormat: isShowdown ? "showdown" : "classic", confidenceRating:round(confidence), contestAggression: settings.aggression, environmentScore: round(environmentScore), boomScore:round(boomScore), bustRisk, bustRiskLabel:bustRisk, bustRiskScore:bustRiskScoreRounded, ownershipLeverageScore:round(leverageScore), exactContestScore, exactContestExplanation, captainScore: isShowdown ? round(captainScore) : 0, captainTier, flexScore: isShowdown ? round(flexScore) : 0, topValueTag:r.value_n>75?"Yes":"No", bestRole:role, tier, contestFit, nonViablePunt, captainBonus: isShowdown ? round(captainBonus) : 0, floorSafetyScore, adjustedFloorRiskScore: round(adjustedFloorRiskScore), classicScore: round(0.38*confidence+0.32*upsideScore+0.18*leverageScore+0.12*r.value_n), explanation};});}
function toPublicResult(r){return { playerName:r.name, team:r.team, position:r.position, salary:r.salary, slateFormat:r.slateFormat, bestRole:r.bestRole, contestFit:r.contestFit, tier:r.tier, captainTier:r.captainTier, confidenceRating:r.confidenceRating, environmentScore:r.environmentScore, boomScore:r.boomScore, bustRisk:r.bustRisk, bustRiskLabel:r.bustRiskLabel, bustRiskScore:r.bustRiskScore, floorSafetyScore:r.floorSafetyScore, adjustedFloorRiskScore:r.adjustedFloorRiskScore, ownershipLeverageScore:r.ownershipLeverageScore, exactContestScore:r.exactContestScore, exactContestExplanation:r.exactContestExplanation, captainScore:r.captainScore, flexScore:r.flexScore, classicScore:r.classicScore, topValueTag:r.topValueTag, explanation:r.explanation };}
